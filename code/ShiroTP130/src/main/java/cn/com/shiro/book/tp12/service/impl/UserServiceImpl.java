...
public class UserServiceImpl implements IUserService {
	...
	public ShrUser createUser(ShrUser user) {
		...
		if (dbUser != null) {
			...
			// 加密
			// 产生一个盐
			ByteSource byteSource = this.randomNumberGenerator.nextBytes();
			String salt = byteSource.toString();
			// 改造盐，获取私盐加密，提高安全性
			int index = this.random.nextInt(5);
			String privateSalt = this.getPrivateSalt(salt, index);
			salt = salt + index;
			// 在密码上加盐
			String saltPassword = privateSalt + user.getUrPassword();
			// 加密
			String password = this.passwordService
					.encryptPassword(saltPassword);
			// 盐和密码更新到user对象
			user.setUrSalt(salt);
	...
	private String getPrivateSalt(String salt, int index) {
		return salt.substring(index, 5);
	}
